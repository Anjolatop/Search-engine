...
def keyword_to_titles(metadata):
    ...
    for i in range(len(metadata)):
        for keyword in metadata[i][4]:
            if keyword in keyword_titles_map:
                keyword_titles_map[keyword].append(metadata[i][0])
            else:
                keyword_titles_map[keyword] = [metadata[i][0]]
    return keyword_titles_map
...
def title_to_info(metadata):
    ...
    for i in range(len(metadata)):
        title_info = {}
        title_info['author'] = metadata[i][1]
        title_info['timestamp'] = metadata[i][2]
        title_info['length'] = metadata[i][3]
        article_title[metadata[i][0]] = title_info
